...
Optional environment:
  ...
  SYMPHONY_ELIXIR_ROOT            Upstream clone path. Default: /Users/ericfode/src/openai-symphony
  ...
  PROJECT_SOURCE_ROOT             Source snapshot root. Default: current repo root
...
export PROJECT_SOURCE_ROOT="${PROJECT_SOURCE_ROOT:-$repo_root}"
...
if [[ ! -d "$symphony_root/.git" ]]; then
  ...
  symphony_status="$(git -C "$symphony_root" status --short)"
  if [[ -z "$symphony_status" ]]; then
    ...
    echo "shared Symphony checkout has local changes; using it without fetch/reset" >&2
  fi
fi
...
port="${SYMPHONY_PORT:-4127}"
sed \
  -e "s/__SYMPHONY_LINEAR_PROJECT_SLUG__/${escaped_slug}/g" \
  -e "s/__SYMPHONY_PORT__/${port}/g" \
  "$workflow_template" > "$generated_workflow"
...
if [[ -n "${SYMPHONY_PORT:-}" || -n "$port" ]]; then
  cmd+=(--port "$port")
fi
...
if [[ "$prepare_only" == "1" ]]; then
  ...
  echo "logs root is $logs_root"
  echo "dashboard port is $port"
  exit 0
fi
