...
test("builds a dataified spec that preserves raw markdown blocks", () => {
  ...
  assert.equal(dataified.schemaVersion, "specgym.dataified-spec.v0");
...
test("loads markdown projection specs and renders a projection", () => {
  ...
  assert.equal(plan.schemaVersion, "specgym.llm-projection-plan.v0");
...
test("builds a trainable projection cycle plan", () => {
  ...
  assert.equal(plan.schemaVersion, "specgym.projection-cycle-training-plan.v0");
